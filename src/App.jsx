...
export default function Game() {
  ...
  const goal=useCallback((side,bx,by)=>{
    ...
    // Simple logic: if ball goes in left goal, right side scores. If in right goal, left scores.
    if(side==='right'){
      // Ball went in LEFT goal (x <= BR) - RIGHT side (player) scores
      const scorer = pm==='single' ? name : 'Player 2';
      const col = myColorRef.current.main;
      showGoalMsg(scorer, col);
      setTimeout(()=>{
        setScore(p=>{const n={...p,right:p.right+1};scoreRef.current=n;return n;});
        resetPos();
      },1200);
    } else {
      // Ball went in RIGHT goal (x >= W-BR) - LEFT side (AI/P1) scores
      const scorer = pm==='single' ? 'AI' : 'Player 1';
      const col = pm==='single' ? oppColorRef.current.main : myColorRef.current.main;
      showGoalMsg(scorer, col);
      setTimeout(()=>{
        setScore(p=>{const n={...p,left:p.left+1}; scoreRef.current=n;return n;});
        resetPos();
      },1200);
    }
